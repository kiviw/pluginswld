 ...
 * Description: A plugin for manual deposit confirmation and KSH disbursement.
 * Version: 1.0.0
 * Author: Your Name
...
function manual_deposit_form_shortcode() {
    ...
    <div class="manual-deposit-form">
        ...
        <form method="post">
            ...
            <label for="tx_hash">Transaction Hash (Unique):</label>
...
function process_deposit_submission() {
    ...
    if ($amount_in_wld <= 0 || empty($phone_number) || empty($mpesa_name) || empty($tx_hash)) {
        echo '<p>Invalid input. Please enter valid WLD amount, phone number, MPESA name, and a unique Transaction Hash.</p>';
        return;
    }
    ...
    // Check if the tx_hash already exists in the database
    $existing_tx = $wpdb->get_var($wpdb->prepare("SELECT id FROM $table_name WHERE tx_hash = %s", $tx_hash));
    if ($existing_tx) {
        echo '<p>Transaction Hash already exists. Please use a unique Transaction Hash.</p>';
        return;
    }
